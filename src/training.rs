...
use burn_backend::backend::Backend;
...
pub fn run<B: Backend>(device: B::Device) {
    ...
    let model = Model::<burn_autodiff::Autodiff<B, NoCheckpointing>>::new(&device);
    ...
    let batcher_train = MnistBatcher::<burn_autodiff::Autodiff<B, NoCheckpointing>>::default();
    let batcher_valid = MnistBatcher::<B>::default();
...
fn evaluate<B: Backend>(
    ...
    let batcher = MnistBatcher::<B>::default();
